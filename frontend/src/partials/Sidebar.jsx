...
function Sidebar({
  ...
  <li key={menu.key}>
    {menu.submenu ? (
      <button
        className={`block w-full text-left text-gray-800 dark:text-gray-100 truncate transition duration-150 ${
          openMenu === menu.key ? "text-violet-500" : "hover:text-gray-900 dark:hover:text-white"
        } mb-2`}
      ...
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center">
            {menu.icon(pathname)}
            <span className="text-sm font-medium ml-4">{menu.label}</span>
          </div>
          <svg
    ...
    {menu.submenu && (
      <ul
        className={`pl-8 mt-1 transition-all duration-200 ${
